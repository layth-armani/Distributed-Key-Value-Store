...
1. become fully acquainted with the project framework and concepts: start by reading [the main description file](/projet/handouts/01.main.md);
...
![A hashtable binds a key to a value using a hashing function](./img/hashtable.png) Fig: _a hashtable binds a key to a value using a hashing function._ (CC) [Jorge Stolfi@WikiMedia](https://commons.wikimedia.org/wiki/User:Jorge_Stolfi)

The concrete representation of the hashtable that we propose to implement in this project will look like this:

![Representation of a hashtable with collision management by chaining](./img/hashtable_with_overflow.png) Fig: _representation of a hashtable with collision management by chaining lists._ (CC) [Jorge Stolfi@WikiMedia](https://commons.wikimedia.org/wiki/User:Jorge_Stolfi)
...
1. define a `kv_pair_t` type that represents a pair (key, value), i.e. a key and a value; although not strictly necessary (we can do without it; it's not as good, but we *can*), this type could be useful when you need to manipulate keys and their associated values as a whole;
1. define in `hashtable.c` (yes, yes, `.c`) the `struct` type associated to (aliased by) the former `typedef`ed `bucked_t`; we indeed recommend you to define is `hashtable.c` and not in `.h` as this type is _strictly internal_, there's no reason to export it via the `.h` file; only its _pre-declaration_ has to be in the `.h` (as you already did); this is an interesting pattern to (1) increase independence (here between hashtable internals, in the `.c` and external usage); and (2) hide implementation details (for whatever reason).
