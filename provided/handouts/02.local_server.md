...
where "`a`", "`bb`", "`ab`" and "`bbb`" are keys, and "`hello xxx`" their corresponding values (see `fake2.h`).
...
Again, from the `done/` directory, you can use the following `make` commands:
